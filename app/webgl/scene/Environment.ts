...
export default class Environment {
  ...
  public updateSkyAndFog(currentFogValue: number, cameraOverlay?: THREE.Mesh) {
    ...
    const targetTop = new THREE.Color()
      .copy(SKY_CLEAN.top)
      .lerp(SKY_POLLUTED.top, ratio);
    const targetMid = new THREE.Color()
      .copy(SKY_CLEAN.mid)
      .lerp(SKY_POLLUTED.mid, ratio);
    const targetBot = new THREE.Color()
      .copy(SKY_CLEAN.bot)
      .lerp(SKY_POLLUTED.bot, ratio);
    const targetFog = new THREE.Color()
      .copy(SKY_CLEAN.fog)
      .lerp(SKY_POLLUTED.fog, ratio);

    const targetDistMin =
      SKY_CLEAN.fogDistMin +
      (SKY_POLLUTED.fogDistMin - SKY_CLEAN.fogDistMin) * ratio;
    const targetDistMax =
      SKY_CLEAN.fogDistMax +
      (SKY_POLLUTED.fogDistMax - SKY_CLEAN.fogDistMax) * ratio;
    const targetDensity =
      SKY_CLEAN.fogDensity +
      (SKY_POLLUTED.fogDensity - SKY_CLEAN.fogDensity) * ratio;

    gsap.to(this.currentSkyState.top, {
      r: targetTop.r,
      g: targetTop.g,
      b: targetTop.b,
      duration: 1.5,
    });
    gsap.to(this.currentSkyState.mid, {
      r: targetMid.r,
      g: targetMid.g,
      b: targetMid.b,
      duration: 1.5,
    });
    gsap.to(this.currentSkyState.bot, {
      r: targetBot.r,
      g: targetBot.g,
      b: targetBot.b,
      duration: 1.5,
    });
    gsap.to(this.currentSkyState.fog, {
      r: targetFog.r,
      g: targetFog.g,
      b: targetFog.b,
      duration: 1.5,
    ...
    gsap.to(this.currentSkyState, {
      ...
      onUpdate: () => {
        ...
        gradient.addColorStop(
          0.7,
          `#${this.currentSkyState.mid.getHexString()}`,
        );
        ...
        if (this.pollutionCloud) {
          ...
          const targetColor = new THREE.Color()
            .copy(SMOKE_COLOR_CLEAN)
            .lerp(SMOKE_COLOR_POLLUTED, ratio);
  ...
  public getFogControls() {
    return this.fogControls;
  }
  public getPollutionCloud() {
    return this.pollutionCloud;
  }
}
